...
const EventManagementPage: React.FC = () => {
    ...
    const currentUserId = user?.token ? (jwtDecode(user.token) as DecodedToken)["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier"] : null;
    ...
    useEffect(() => {
        ...
        const fetchManagementData = async () => {
            ...
            try {
                ...
                const regRes = await fetch(`${API_BASE_URL}/api/registrants?eventId=${eventId}`, {
                    headers: {'Authorization': `Bearer ${user.token}`}
                });
                if (!regRes.ok) {
    ...
    const handleDownloadInvoice = async (invoiceId: number) => {
        ...
        try {
            ...
            if (!response.ok) {
                toast.error("Could not download the invoice.", {id: loadingToast});
                return;
            }
            ...
            <div className="detail-page-container fade-in">
                ...
                <div className="detail-page-grid">
                    <div className="page-section">
                        <div className="section-header"><Skeleton className="h2-skeleton"/></div>
                        <div className="event-details-list"><Skeleton className="list-item-skeleton"/><Skeleton
                            className="list-item-skeleton"/><Skeleton className="list-item-skeleton"/></div>
                    </div>
                    <div className="page-section">
                        <div className="section-header"><Skeleton className="h2-skeleton"/></div>
                        <div className="attendee-list"><Skeleton className="attendee-item-skeleton"/><Skeleton
                            className="attendee-item-skeleton"/><Skeleton className="attendee-item-skeleton"/></div>
                    </div>
                </div>
            </div>
        );
        ...
        <div className="detail-page-container fade-in">
            ...
            <div className="detail-page-grid">
                <section className="page-section">
                    <div className="section-header"><h2>Event Details</h2></div>
                    <div className="event-details-list">
                        <div className="event-detail-item"><span>🗓️</span>
                            <div><strong>Date:</strong> {new Date(event.date).toLocaleDateString()}</div>
                        </div>
                        <div className="event-detail-item"><span>📍</span>
                ...
                <section className="page-section">
                    <div className="section-header"><h2>Attendees ({registrants.length})</h2></div>
